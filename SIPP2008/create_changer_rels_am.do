...
reshape long SHHADID adj_age arrivers leavers stayers comp_change comp_change_reason, i(SSUID EPPPNUM) j(panelmonth)
...
save "$tempdir/comp_change_onlychangers_am", $replace
...
foreach changer in leaver arriver stayer {
    ...
    use "$tempdir/comp_change_onlychangers_am"
    ...
    * Compute max number of leavers/arrivers.
    ...
    keep SSUID EPPPNUM SHHADID panelmonth adj_age comp_change_reason n_`changer's `changer'* 

    reshape long `changer', i(SSUID EPPPNUM panelmonth) j(`changer'_num)

    drop if missing(`changer')

    save "$tempdir/hh_`changer's_am", $replace
}
...
use "$tempdir/hh_leavers_am", clear
...
save "$tempdir/leaver_rels_am", $replace
...
use "$tempdir/hh_arrivers_am", clear
...
tab relationship, m sort
save "$tempdir/arriver_rels_am", $replace
...
foreach changer in leaver arriver {
	...
	* get changer age *
    gen EPPPNUM = relto
    merge m:1 SSUID EPPPNUM panelmonth using "$SIPP08keep/demo_long_all_am", keepusing(adj_age)
    drop if (_merge == 2)
    assert (_merge == 3)
	...
	* bring back ego's person number
	...
	save "$tempdir/`changer'_rels_withage_am", $replace
}    

gen change_type=1

append using "$tempdir/leaver_rels_withage_am"
...
save "$tempdir/changer_rels_am", $replace
